...
2. **apps/worker** — Python 3.11, consome a fila `jobs` (Postgres, sem Redis
   ...
   copy (Ollama + fallback), geração de card (Pictify), publicação.
...
[worker] 3 cópias (ollama -> fallback) -> contents PENDING_REVIEW
...
| Pictify (cards) | 1 | free tier (50 imagens/mês) |
...
- **Ollama local + fallback determinístico**: copy nunca depende de API
  paga nem inventa fato; provider abstrato para trocar depois.
